...
//   Origin: src/plugin-sdk/plugin-entry.ts (provider plugin shape)
...
export type PluginLLMStreamChunk =
  ...
  | { type: 'usage'; usage: PluginLLMUsage }
  | { type: 'finish'; finish_reason: PluginLLMCompletionResponse['finish_reason']; usage?: PluginLLMUsage };

/** Result type (mirrors Alduin's Result<T, E>). */
export type PluginResult<T, E> = { ok: true; value: T } | { ok: false; error: E };
